// lib/core/parser/parsers/isbankasi_statement_parser.dart

import '../models/parsed_models.dart';
import '../../utils/currency_normalizer.dart';

class IsBankasiStatementParser {
  // Kart veya Müşteri Başlığı
  static final RegExp _cardHeaderRegex = RegExp(
    r'(?:Kart Numarası|Kart No|Hesap No)\s*:\s*([\d\* ]+)(?:\s+([A-ZÇĞİÖŞÜa-zçğıöşü ]+))?',
    caseSensitive: false,
  );

  // İş Bankası Standart Satır: "11.08.2026 BOYNER BUYUK MAGAZACILIK 2.400,00 TL (2/4 TAKSIT)"
  ...
  List<ParsedRecord> parse(String text, {String? accountMask}) {
    ...
    String currentAccountMask = accountMask ?? 'TR.. 0064 **** ****';
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (txMatch != null) {
        ...
        if (lowerDesc.contains('dönem borcu') ||
            ...
            lowerDesc.contains('kullanılabilir')) {
          continue;
        }

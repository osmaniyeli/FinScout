// lib/features/tax_analytics/services/tax_calculator_service.dart
  ...
  TaxBreakdown calculateMonthlyTaxLoad({
    ...
    for (final tx in transactions) {
      ...
      // 3. Tüketimden Doğan Tahmini KDV Hesabı (Kategori bazlı varsayılan oranlar)
        ...
        // Temel Gıda & Market: Ağırlıklı ortalama ~%8 KDV
        estimatedVat += (amount * 0.08 / 1.08).round();
      } else if (category == 'cat_utilities') {
        // Fatura / Telekomünikasyon / Enerji: ~%20 KDV
        estimatedVat += (amount * 0.20 / 1.20).round();
      } else if (category != 'cat_salary') {
        // Genel tüketim: ~%20 KDV varsayımı
        estimatedVat += (amount * 0.20 / 1.20).round();
      }
    }

    final int totalStateShare = directPayrollTaxesCents + financialLevies + officialFees + estimatedVat;
    final double stateShareRatio = totalExpenditureCents > 0
    ...
    return TaxBreakdown(
      ...
      stateShareRatio: double.parse(stateShareRatio.toStringAsFixed(1)),
    );
  }
}
